...
const NpkCalcPage = () => {
  ...
  const [phosphor, setPhosphor] = useState(0)
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      const response = await customFetch('/analysis/create', 'POST', true, {
        ...data,
        isCalculateNpk: true,
        phosphor,
        potassium,
      ...
      const nitrogen = await response.analysis.nitrogen

      if (response.success) {
        toast.success('Analise cadastrada!', {
          duration: 5000,
          position: 'top-center',
          classNames: {
            toast: 'bg-white',
